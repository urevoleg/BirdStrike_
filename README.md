...
В модуле config.py содержатся данные для создания клиентов для подключений к базам данных, пока что это только подключение к POSTGRESQL
Код создания клиентов для подключений записан в модуле connections.py
...
Заполнение STAGE слоя
    ...
    Выполняется загрузка данных о погоде
        Это самая долгая часть работы. Так как на сайте National Centers for Environmental Information подробные сведения о погоде хранятся по годам и по станциям.
        Обобщенные данные по станциям очень ограничены. Поэтому при создании базы данных, в нее сразу же заливается пулл последних актуальных данных по станциям (индивидуальный номера)
        ...
        Далее через selenium webdriver выполняется скачивание файла с наблюдениями за год для каждой станции. В будущем этот этап следует распаралелить на несколько парралельных обработок.
    ...
    Выполняется загрузка данных о авиационных инцидентах с участием животных
        ...
        **Важно 6 за 2018 год станций не получается прогрузить. Придется смотреть что происходит и улучшить систему логгирования**
